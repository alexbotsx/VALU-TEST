...
export default async function importLoader(module) {
    ...
    //         workerData: module
    //     })
    //     const SkyBOTWorker = () => worker.terminate().catch(() => { })
    //     worker.once('message', (msg) => (SkyBOTWorker(), console.log(msg.data), resolve(msg)))
    //     worker.once('error', (error) => (SkyBOTWorker(), reject(error)))
    // })
